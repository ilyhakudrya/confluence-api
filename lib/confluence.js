 ...
 * @param {boolean} [config.useApiV1] - use REST API V1
...
function Confluence(config) {
    ...
    this.config.apiPath = '/api/v2';

    if (this.config.useApiV1) {
...
Confluence.prototype.getSpaces = function(callback) {
    request
        .get(this.config.baseUrl + this.config.apiPath + '/spaces')
 ...
 * @param {Function} callback
 */
Confluence.prototype.getSpaceById = function(id, callback) {
    request
        .get(this.config.baseUrl + this.config.apiPath + '/spaces/' + id + '?body-format=storage')
        .auth(this.config.username, this.config.password)
        .end(function(err, res){
            processCallback(callback, err, res);
        });
 ...
 * @param {String} data.spaceId - ID of the space
 * @param {String} data.status - the status of the page, published or draft
 * @param {String} data.title - title of the page, required if page status is not draft
 * @param {String} data.parentId - the parent content ID of the page
 * @param {Object} data.body
 * @param {String} data.body.representation - type of content representation used for the value field. Valid values: storage, atlas_doc_format, wiki
 * @param {String} data.body.value - body of the page, in the format found in the representation field
 * @param {Function} callback
 */
Confluence.prototype.createPage = function(data, callback) {
    request
        .post(this.config.baseUrl + this.config.apiPath + '/pages')
        .auth(this.config.username, this.config.password)
        .type('json')
        .send(data)
        .end(function(err, res){
 ...
 * REST API V2
 *
 * @param {Object} id - id of the page
 * @param {Function} callback
 */
Confluence.prototype.getPageById = function(id, callback) {
    request
        .get(this.config.baseUrl + this.config.apiPath + '/pages/' + id + '?body-format=storage')
        .auth(this.config.username, this.config.password)
        .end(function(err, res){
            processCallback(callback, err, res);
        });
 ...
 * @param {String} data.id - ID of the page
 * @param {String} data.status - the updated status of the page.
 ...
 * @param {Number} data.version.number - the new version of the updated page
 ...
 * @param {String} data.version.message - an optional message to be stored with the version
 * @param {Function} callback
 */
Confluence.prototype.updatePage = function(data, callback) {
    request
        .put(this.config.baseUrl + this.config.apiPath + '/pages/' + data.id)
        .auth(this.config.username, this.config.password)
        .type('json')
        .send(data)
        .end(function(err, res){
 ...
 * @param {Function} callback
 */
Confluence.prototype.deletePage = function(id, callback) {
    request
        .delete(this.config.baseUrl + this.config.apiPath + '/pages/' + id)
        .auth(this.config.username, this.config.password)
        .end(function(err, res){
            processCallback(callback, err, res);
        });
 ...
 * @param {Function} callback
 */
Confluence.prototype.getPageVersions = function(id, callback) {
    request
        .get(this.config.baseUrl + this.config.apiPath + '/pages/' + id + '/versions?limit=250')
        .auth(this.config.username, this.config.password)
        .end(function(err, res){
            processCallback(callback, err, res);
        });
